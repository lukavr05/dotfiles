...
# Get current weather
current_weather=$(curl -sf "http://api.openweathermap.org/data/2.5/weather?id=$CITY_ID&appid=$API_KEY&units=$UNITS")

# Get forecast data
forecast_data=$(curl -sf "http://api.openweathermap.org/data/2.5/forecast?id=$CITY_ID&appid=$API_KEY&units=$UNITS")
...
if [ ! -z "$current_weather" ] && [ ! -z "$forecast_data" ]; then
    ...
    output="$DISPLAY_NAME %{T2}$current_icon%{T-} ${current_temp}°C"
    ...
    echo "$DISPLAY_NAME - Weather Forecast Unavailable"
fi
